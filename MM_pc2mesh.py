...
import PIL.Image
import py3dtiles
import os, platform, shutil, zipfile, logging, sys, glob, utm, time, pymeshlab, psutil, threading, copy, cv2
from tkinter import *
...
class pc2mesh():
    ...
    def process_exlog(self, exlog_path):
        ...
        os.mkdir(scan_log_image_folder+"/"+exlog_file_name)
        
        for png_file in png_files:
            
            shutil.copy(png_file, scan_log_image_folder+"/"+exlog_file_name)
    ...
    def get_file(self):
        ...
        global exlog_name_for_images, file_ext_check, file_origin, zip_file_for_compression, raw_obj, server_addr, resulting_mesh_type, auto_open, swap_axis, path, filename, mesh_output_folder, gen_path_folder, simplified_output_folder, with_texture_output_folder, obj_file, separator, c, log_name, lat, lon, elev, utm_easting, utm_northing, zone, log_name, log_folder, pc_folder, post_dest_folder, scan_log_image_folder, model_dest_folder, face_number, designator, folder_type, folder_suffix, texture_size
        ...
        scan_log_image_folder = "ARTAK_MM/DATA/Imagery"
